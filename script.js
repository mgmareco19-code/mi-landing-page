...
(function () {
  ...
  // Modo oscuro: recuerda la preferencia del usuario en este navegador
  function initThemeToggle() {
    const btn = document.getElementById('themeToggle');
    if (!btn) return;
    const root = document.documentElement;
    const STORAGE_KEY = 'apw-theme';

    let stored = null;
    try { stored = localStorage.getItem(STORAGE_KEY); } catch (e) { /* localStorage no disponible */ }
    ...
    btn.addEventListener('click', () => {
      ...
      applyTheme(nowDark);
      try { localStorage.setItem(STORAGE_KEY, nowDark ? 'dark' : 'light'); } catch (e) { /* ignorar */ }
    });
